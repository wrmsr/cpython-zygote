...
#define BASE_OPTS "3bBc:dEhiJm:Op:Q:RsStuUvVW:xX?"
...
-p sz  : permit heap pinning, reserving sz bytes (or none if sz is zero)\n\
...
Py_Main(int argc, char **argv)
{
    ...
    while ((c = _PyOS_GetOpt(argc, argv, PROGRAM_OPTS)) != EOF) {
        ...
        case 'p':
            // FIXME fuck you
            if ((pinreserve = PyOS_strtoul(_PyOS_optarg, NULL, 0)) < 0) {
                fprintf(stderr, "invalid pinreserve\n");
                return 1;
            }
            break;
    ...
    if (pinreserve >= 0) {
        _PyMem_SetupContiguousAllocation(pinreserve);
    }
